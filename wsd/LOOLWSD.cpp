...
#include <sqlite3.h>
#include <curl/curl.h>  // -lcurl 會與 online 衝突
...
std::string LogFilePath;  // for /lool/convertto
...
void LOOLWSD::defineOptions(OptionSet& optionSet)
{
    ...
    optionSet.addOption(Option("maxmac", "", "Max number of mac address to control for /lool/convert-to.")
                        .required(false)
                        .repeatable(false));


    optionSet.addOption(Option("maxip", "", "Max number of ip address to control for /lool/convert-to.")
                        .required(false)
                        .repeatable(false));
    ...
    void handleIncomingMessage(SocketDisposition &disposition) override
    {
        ...
        try
        {
            ...
            if (logger.enabled())
            {
                ...
                //fprintf(stderr, "cookie: %s\turi:%s\n", cookie.c_str(), request.getURI().c_str());
                LOG_END(logger);
            }
    ...
    const std::string TEMPL = R"MULTILINE(
{
    ...
    const std::string YAMLTEMPL = R"MULTILINE(swagger: '2.0'
info:
  version: v1
  title: 轉檔管理 & ODF 報表 API
  description: ''
...
paths:%s)MULTILINE";
        ...
        if (!showHead && yaml)
            paths = Poco::format(YAMLTEMPL, ServerName, paths);
                           ...
                           SocketDisposition &disposition)
    {
        ...
        if (tokens.count() == 3 && tokens[2] == "convert-to")
        {
            ...
            if (!fromPath.empty())
            {
                if (!format.empty())
                {
                    ...
                    if (clientSession)
                    {
                        if (queue)  // set queue for file download
                            clientSession->setQueue(format);
                        ...
                        docBroker->addCallback([docBroker, moveSocket, clientSession, format]()
                        {
                            ...
                            fprintf(stderr, "%s\n", docBroker->getPublicUri().getPath().c_str());
        ...
        {
            fprintf(stderr, "File download request.");
            ...
            {
                response.setStatusAndReason(HTTPResponse::HTTP_BAD_REQUEST, "Invalid or unknown request.");
                // 當uri=http://uri/lool/convert-to/api///
                // （也就是後面有兩個以上斜線）流程會到這裡，
                // 這邊加上 send socket 使 client 呼叫後能取回回應
                socket->send(response);
                socket->shutdown();
                                        ...
                                        (const std::shared_ptr<Socket> &moveSocket)
                    {
                        ...
                        docBroker->addCallback([docBroker, moveSocket, clientSession]()
                        {
                            ...
                            catch (const StorageConnectionException& exc)
                            {
                                ...
                                const std::string msg = "error: cmd=storage kind=loadfailed msg=" + std::string(exc.what());
                                clientSession->sendMessage(msg);
                            }
                            catch (const std::exception& exc)
                            {

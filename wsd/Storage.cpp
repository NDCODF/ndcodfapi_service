...
bool StorageBase::WebDAVEnabled;
Util::RegexListMatcher StorageBase::WopiHosts;
Util::RegexListMatcher StorageBase::OtherHosts;
...
void StorageBase::initialize()
{
    ...
    WebDAVEnabled = app.config().getBool("storage.webdav[@allow]", false);
...
std::unique_ptr<StorageBase> StorageBase::create(const Poco::URI& dummyUri, const std::string& jailRoot, const std::string& jailPath)
{
    ...
    Poco::URI::decode(dummyUri.toString(), decodedUri);

    Poco::URI newuri(decodedUri);
    ...
    else if (WopiEnabled)
    {
        ...
        if (WopiHosts.match(targetHost) || Util::isLocalhost(targetHost))
        {
    ...
    {
        LOG_INF("Public URI [" << uri.toString() << "] considered WebDAVStorage.");
        return std::unique_ptr<StorageBase>(new WebDAVStorage(uri, jailRoot, jailPath));
    }
...
std::unique_ptr<WopiStorage::WOPIFileInfo> WopiStorage::getWOPIFileInfo(const std::string& accessToken)
{
    ...
    return std::unique_ptr<WopiStorage::WOPIFileInfo>(new WOPIFileInfo({userId, userName, canWrite, postMessageOrigin, hidePrintOption, hideSaveOption, hideExportOption, enableOwnerTermination, disablePrint, disableExport, disableCopy, filename, callDuration}));
}

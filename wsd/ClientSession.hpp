...
public:
    ...
    /// for convert-to: set queue and doc format
    void setQueue(std::string format)
    {
        _isQueue = true;
        _queueFormat = format;
    }
...
private:
    ...
    bool _isQueue;  // convert-to: queue parameter setted.

    std::string _queueFormat;  // convert-to: queue parameter setted.
};

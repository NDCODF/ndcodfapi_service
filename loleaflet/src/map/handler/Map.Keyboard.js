...
L.Map.Keyboard = L.Handler.extend({
	...
	_isComposing: false, // IME Composing going on
	hasCompositionJustEnded: false, // Used to swallow keyup event related to compositionend
	...
	_onKeyDown: function (e) {
		...
		if (e.type === 'compositionstart' || e.type === 'compositionupdate') {
			...
			// 組字區寬度隨組字字數變動(chrome 除外)
			if (!navigator.userAgent.toLowerCase().match(/chrome\/([\d.]+)/)) {
				$('.clipboard').css('width', $('.clipboard').height() * e.originalEvent.data.length + 'px');
			}

			/* 開啟組字區區塊 */
			$('.clipboard').css('opacity', 1).zIndex(100);
			$('.clipboard-container').zIndex(100);
		}
		var compCharCodes = [];
		if (e.type === 'compositionend' || e.type === 'textInput') {
			this._isComposing = false; // stop of composing with IME
			// get the composited char codes
			for (var i = 0; i < $('.clipboard').val().length; i++) {
				compCharCodes.push($('.clipboard').val()[i].charCodeAt());
			}

			/* 關閉組字區區塊，防止滑鼠點到 */
			$('.clipboard').css('opacity', 0).zIndex(-1);
			$('.clipboard-container').zIndex(-1);
		...
		if (this._map._permission === 'edit') {
			...
			// 組字區位置, 隨 writer/calc/impress/draw 不同
			...
			if (window.navigator.userAgent.indexOf('Trident/') > 0)
			{  // IE11
				ofx = parseInt($('.leaflet-pane').css('msTransform').split(',')[12]);
				ofy = parseInt($('.leaflet-pane').css('msTransform').split(',')[13]);
			}
			else
			{
				ofx = parseInt($('.leaflet-pane').css('transform').split(',')[4]);
				ofy = parseInt($('.leaflet-pane').css('transform').split(',')[5]);
			}
			var cursorPos = this._map.latLngToLayerPoint(this._map._docLayer._visibleCursor.getNorthWest());
			$('.clipboard-container').css({
				left: cursorPos.x + distance + ofx + 'px',
				top: $('#document-container').position().top + ofy +cursorPos.y + 'px'});

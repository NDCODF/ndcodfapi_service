...
L.Socket = L.Class.extend({
	...
	initialize: function (map) {
		...
		try {
			...
			console.log(map.options.doc + '?' + $.param(map.options.docParams));
	...
	_onMessage: function (e) {
		...
		if (textMsg.startsWith('loolserver ')) {
			...
			global.bundle = loolwsdVersionObj.Bundle;
			...
			if (perm === 'readonly') {
				...
				this._map.allowedViewModeActions = JSON.parse(perm);
				console.log(this._map.allowedViewModeActions);
				perm = this._map.allowedViewModeActions['perm'];
			}
			...
			console.log('permission from WOPI permission:', perm)
			...
			this._map.fire('toolbarperm', this._map.allowedViewModeActions['toolbar']);
		...
		else if (textMsg.startsWith('ossii: ')) {
			global.title = textMsg.replace('ossii: ', '');
			return;
		}
		else if (textMsg.startsWith('ex500retrytimes: ')) {
			this.ex500retrytimes = parseInt(textMsg.replace('ex500retrytimes: ', ''));
			return;
		}
		else if (textMsg.startsWith('ex500retrydelay: ')) {
			this.ex500retrydelay = parseInt(textMsg.replace('ex500retrydelay: ', ''));
			return;
		}
			...
			else if (command.errorKind === 'exexception500') {
				storageError = command.errorMsg;
				this._map._active = false;
				if (++this.FromOSSII < this.ex500retrytimes) {
					this._reconnecting = false;
				}
				else {
					this._reconnecting = true;
					this.ExException = true;
					this._map.hideBusy();
					this.FromOSSII = 0;
					this._map.fire('warn', {msg: storageError, kind: 'ex'});
				}
				return;
			}
			else if (command.errorKind === 'exsaveexception') {
				storageError = command.errorMsg;
			}
			else if (command.errorKind === 'exexception') {
				storageError = command.errorMsg;
				this._reconnecting = false;
				this.ExException = true;
				this._map.hideBusy();
				this._map.fire('error', {msg: storageError, kind: 'ex'});
			}
				...
				vex.dialog.open({
					...
					callback: L.bind(function(data) {
						if (data) {
							...
							this._map.options.docParams['docpass'] = 'yes';
							this.initialize(this._map);
						} else if (passwordType === 'to-modify') {
		...
		else if (textMsg.startsWith('error:') && command.errorCmd === 'worker') {
			var msg = decodeURI(command.errorKind);
			this._map._fatal = true;
			this._map.fire('error', {msg: _(msg)});

			if (this._map._docLayer) {
				this._map._docLayer.removeAllViews();
			}
			this.close();
			this._map.showBusy(_(msg), false);
			return;
		}
	...
	_onSocketClose: function (e) {
		...
		if (isActive && this._reconnecting && !this.ExException) {
		...
		if (!this._reconnecting && !this.ExException) {
			var that = this;
			this.vexTimer = setTimeout(function() {
				that._reconnecting = true;
				that._map.hideBusy();
				that._map._activate();
			}, this.ex500retrydelay);
		}
		if (this.ExException)
			this.ExException = false;
	},
	...
	parseServerCmd: function (msg) {
		...
		for (var i = 0; i < tokens.length; i++) {
			...
			/*else if (tokens[i].substring(0, 6) === 'isPdf=') {
				alert('pdf');
				command.isPdf = true;
			}*/
			...
			else if (tokens[i].substring(0, 4) === 'msg=') {
				command.errorMsg = tokens[i].substring(4);
			}

...
L.TileLayer = L.GridLayer.extend({
	...
	initialize: function (url, options) {
		...
		// query doctype
		var fname = global.title.split(',')[1];
		if (fname)
		{
			fname = fname.split('.');
			if (fname.length > 1)
			...
			parent.postMessage('{"type": "OXOOL_DOCINFO", "doctype": "' + options.docType +'", "filename": "' + fname +'"}', '*');
		}
	},
	...
	_onDownloadAsMsg: function (textMsg) {
		...
		if (command.id === 'print') {
			if (L.Browser.gecko || L.Browser.edge || L.Browser.ie || this._map.options.print === false) {
				// the print dialog doesn't work well on firefox/IE
	...
	_onStateChangedMsg: function (textMsg) {
		...
		/* 組字區長寬隨字型大小及文件縮放(zoom)變動 */
		if (commandName === '.uno:FontHeight') {
			console.log(state);
			var scale = $('#zoomlevel.loleaflet-font').html().replace('%', '') / 100;
			console.log(scale);
			$('.clipboard').css('font-size', state * scale + 'pt');
			$('.clipboard').css('height', 'auto');
			$('.clipboard').css('height', $('.clipboard').height()/* / 2*/ + 'px');
		}
	...
	_onTextSelectionEndMsg: function (textMsg) {
		var strTwips = textMsg.match(/\d+/g);
		if (strTwips != null && this._map._permission === 'edit'/* || this._map._permission === 'view'*/) {
			var topLeftTwips = new L.Point(parseInt(strTwips[0]), parseInt(strTwips[1]));
			var offset = new L.Point(parseInt(strTwips[2]), parseInt(strTwips[3]));
			var bottomRightTwips = topLeftTwips.add(offset);
	...
	_updateCursorAndOverlay: function (update) {
		if ((this._map._permission === 'edit'/* || this._map._permission === 'view'*/)
	...
	_onUpdateGraphicSelection: function () {
		if (this._graphicSelection && !this._isEmptyRectangle(this._graphicSelection)) {
			...
			if (!this._graphicMarker) {
				console.log('error: Graphic marker initialization');
				return;
			}
	...
	_onUpdateCellCursor: function (horizontalDirection, verticalDirection, onPgUpDn) {
		if (this._cellCursor && !this._isEmptyRectangle(this._cellCursor)) {
			...
			this._cellCursorMarker = L.rectangle(this._cellCursor, {
				...
				weight: 2});
			if (!this._cellCursorMarker) {
				console.log('error: Cell Cursor marker initialization');
				return;
			}
	...
	_resize: function () {
		var _calcTitle = function (maxlen) {
			...
			if (dotPos > -1)
			{
				ext = tmp.substr(dotPos, tmp.length);
				body = tmp.substr(0, dotPos);
			}
		...
		var title;
		try {
			title = decodeURIComponent(buf.join(''));
		} catch (e) {
			title = buf.join('');
		}

				...
				return {
					callback: function(key, t) {
						...
						if (key === '.uno:Paste')  {
							...
							{
								vex.dialog.open({
									message: '不允許 Firefox 或 Chrome 以選單執行貼上動作<br \>請改以鍵盤按鍵 Ctrl+V 觸發',
									buttons: [
								...
								map._socket.sendMessage('paste mimetype=text/plain;charset=utf-8\n' + pasteString);
							}
						}
						/*else if (key === '.uno:InsertAnnotation') {
							console.log(key);
							comment = {
								Text: {
									type: 'string',
							...
							map.sendUnoCommand(key, comment);
						}*/
						else

...
$(function () {
	...
	$('#spreadsheet-toolbar').w2toolbar({
		...
		items: [
			{type: 'button',  id: 'firstrecord',  img: 'firstrecord', hidden: true, hint: _('First page')},
			{type: 'button',  id: 'prevrecord',  img: 'prevrecord', hidden: true, hint: _('Previous page')},
			{type: 'button',  id: 'nextrecord',  img: 'nextrecord', hidden: true, hint: _('Next page')},
			{type: 'button',  id: 'lastrecord',  img: 'lastrecord', hidden: true, hint: _('Last page')}
		],
		onClick: function (e) {
			onClick(e.target);
		}
...
function onStyleSelect(e) {
	...
	if (style.startsWith('.uno:') && style !== '.uno:ResetAttributes') {
		...
		if (style !== '.uno:ResetAttributes')  // 清除段落格式
			map.applyStyle(style, 'ParagraphStyles');
		else
			map.applyStyle('Standard', 'ParagraphStyles');
	}
...
map.on('toolbarperm', function(e) {
	var disabledIds = map.allowedViewModeActions['toolbar'];
	for (var idx in disabledIds)
	{
		var id = disabledIds[idx];
		w2ui['toolbar-up'].remove(disabledIds[idx]);
		w2ui['toolbar-up-more'].remove(disabledIds[idx]);
		if (id === 'formulabar')
		{
			var toolbar = w2ui.formulabar;
			toolbar.remove('sum');
			toolbar.remove('function');
			toolbar.remove('cancelformula');
			toolbar.remove('acceptformula');
			toolbar.remove('formula');
		}
	}
});
...
map.on('doclayerinit', function () {
	...
	var $pathIco = $('div#pinfo').attr('name');
	...
	case 'spreadsheet':
		...
		toolbarUpMore.remove('inserttable', 'styles', 'justifypara', 'defaultbullet', 'defaultnumbering', 'break-numbering', 'insertfootnote', 'footnotebreak');
		statusbar.disable('zoomreset', 'zoomout', 'zoomin', 'zoomlevel');
		statusbar.set('next', {hint: _('Next sheet')});
		statusbar.set('prev', {hint: _('Previous sheet')});
		statusbar.insert('left', [
			...
			{type: 'html',  id: 'StateTableCell',
				...
				{ func: '1', text: _('None'), icon: 'selected'},
		]},
		]);
	...
	case 'presentation':
		...
		toolbarUp.remove('logo_odt', 'logo_ods', 'insertannotation', 'wraptextseparator', 'wraptext', 'togglemergecells', 'break-toggle', 'numberformatcurrency', 'numberformatpercent', 'numberformatdecimal', 'numberformatdate', 'numberformatincdecimals', 'numberformatdecdecimals', 'break-number', 'sortascending', 'sortdescending', 'repair', 'insertfootnote', 'footnotebreak');
		toolbarUpMore.remove('logo_odt', 'logo_ods', 'insertannotation', 'wraptextseparator', 'wraptext', 'togglemergecells', 'break-toggle', 'numberformatcurrency', 'numberformatpercent', 'numberformatdecimal', 'numberformatdate', 'numberformatincdecimals', 'numberformatdecdecimals', 'break-number', 'sortascending', 'sortdescending', 'repair', 'insertfootnote', 'footnotebreak');
		statusbar.insert('left', [
			{type: 'break', id:'break1'},
	...
	if (map.isPdf()) {
		$('div#logo').addClass('logo_pdf');
		$('div#logo_wrap').addClass('logo_wrap_pdf');
	}
	toolbarUp.refresh();
...
map.on('commandstatechanged', function (e) {
	...
	if (commandName === '.uno:AssignLayout') {
		...
		if (map._permission === 'view') {
			$('#tb_toolbar-down_item_break2').remove();
			$('#tb_toolbar-down_item_break3').remove();
			$('#tb_toolbar-down_item_break5').remove();
		}
	}
		...
		if (map._permission === 'view') {
			$('#tb_toolbar-down_item_break2').remove();
			$('#tb_toolbar-down_item_break5').remove();
			$('#tb_toolbar-down_item_break6').remove();
		}
	}
	else if (commandName === '.uno:StateWordCount' && map._permission !== 'view') {
...
function updateCommandValues() {
	...
	if ($('.fonts-select option').length === 0) {
		...
		$('.fonts-select').select2({  // 字型列表依照中文優先排序
...
map.on('addview', function(e) {
	...
	if (e.bundle === 'of') {
		...
		w2ui['toolbar-down'].hide('userlist');
		userlistid = 'li.menu-usermenu';
	}
	$(userlistid)
		.w2overlay({
	...
	if (e.bundle === 'of') {
		newhtml = '<li id="user-' + e.viewId + '" class="item">' +
				'<div class="usercolor" style="background-color:'+ color + ';"></div>' +
				'<a class="username">'+username+'</a>' +
				'<span class="hotkey"></span>' +
				'</li>';
		$('li.menu-usermenu').find('.ulmenu').append(newhtml);
	}
...
map.on('removeview', function(e) {
	...
	// 上方使用者列表: remove
	$('li.menu-usermenu').find('#user-' + e.viewId).remove();
	// 下方使用者列表: remove
	var userlistItem = w2ui['toolbar-down'].get('userlist');
